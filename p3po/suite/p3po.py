...
class P3POWrapper(dm_env.Environment):
    def __init__(
        ...
        self.write_videos = write_track_video
        self.video_list = []
    ...
    def reset(self, **kwargs):
        ...
        if obs[self.pixel_keys[0]].shape[0] == 3:
            ...
            # obs[self.pixel_keys[0]] = obs[self.pixel_keys[0]], (1, 2, 0)
        ...
        if self.write_videos:
            img_plots = self.points_class.plot_image()
            self.video_list.append(img_plots[0])
    ...
    def step(self, action):
        ...
        if obs[self.pixel_keys[0]].shape[0] == 3:
            obs[self.pixel_keys[0]] = np.transpose(obs[self.pixel_keys[0]], (1, 2, 0))
            # obs[self.pixel_keys[0]] = obs[self.pixel_keys[0]]
        ...
        if len(self.depth_keys) > 0 and self.depth_keys[0] in obs:
            ...
            print(f"Depth key: {self.depth_keys[0]} not found")
