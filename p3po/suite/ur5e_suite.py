...
class RGBArrayAsObservationWrapper(dm_env.Environment):
    ...
    def reset(self, **kwargs):
        ...
        obs = {}
        env_obs = self._env.reset(**kwargs)
        # step_obs,reward, _, done, info = self._env.step(np.zeros(self.action_space.shape))
        obs["features"] = env_obs["features"]
    ...
    def step(self, action):
        ...
        obs["features"] = observation["features"]
        if len(self.depth_keys) > 0:
            obs["depth"] = self.get_depth()
